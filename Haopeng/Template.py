...
def exists_gain(e, b_id, s1_id, s2_id):
    ...
    if book_b and book_s1.bids and book_s2.bids and book_b.asks:
        ...
        minimo_stocks_unit = min(book_s1.asks[0].volume,book_s2.asks[0].volume)
        n = min((book_b.bids[0].volume // 2 * 2)/2,minimo_stocks_unit)
        gain = (n*book_s1.bids[0].price + n*book_s2.bids[0].price) - (2*n*book_b.asks[0].price)
        ...
        if (gain > 0.1):
            ...
            return [0, book_b.asks[0].price, 2*n, book_s1.bids[0].price, n, book_s2.bids[0].price, n, gain]
            
    if book_b and book_s1.asks and book_s2.asks and book_b.bids:
        # Comprar basket y vender stock
        minimo_stocks_unit = min(book_s1.asks[0].volume,book_s2.asks[0].volume)
        minimo_b_unit = min(2*minimo_stocks_unit,book_b.bids[0].volume // 2 * 2)
        n = min(minimo_b_unit/2,minimo_stocks_unit)
        gain = (2*minimo_b_unit*book_b.bids[0].price) - (n*book_s1.asks[0].price + n*book_s2.asks[0].price)
        if (gain > 0.1):
            ...
            return [1, book_b.bids[0].price, 2*n, book_s1.asks[0].price, n, book_s2.asks[0].price, n, gain]
...
def main():
    ...
    while True:
        ...
        if result[0]==0:
            ...
            response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1], volume=volume_basket, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
            print_order_response(response)
            response: InsertOrderResponse = exchange.insert_order(case[1], price=result[3], volume=volume_stock, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
            print_order_response(response)
            response: InsertOrderResponse = exchange.insert_order(case[2], price=result[5], volume=volume_stock, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
            
            print("CASE 1", exchange.get_positions_and_cash())
            ...
            if positions_and_cash[case[0]]["volume"] - volume_basket <= -450:
                response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1], volume=volume_basket, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
                print_order_response(response)
            ...
            if positions_and_cash[case[1]]["volume"] + volume_stock >= 450:
                response: InsertOrderResponse = exchange.insert_order(case[1], price=result[3], volume=volume_stock, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
                print_order_response(response)
            ...
            if positions_and_cash[case[2]]["volume"] + volume_stock >= 450:
                response: InsertOrderResponse = exchange.insert_order(case[2], price=result[5], volume=volume_stock, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
                print_order_response(response)
                
            ''' Keep position around 0
        ...
        if result[0]==1:
            ...
            print("CASE 2", exchange.get_positions_and_cash())
