...
def main():
    ...
    while True:
        ...
        '''
        for instr in exchange.get_instruments():
            book = exchange.get_last_price_book(instr)
            print(book.instrument_id)
            print(book.asks)
            print(book.bids)
        
        response: InsertOrderResponse = exchange.insert_order("C1_GAS_INC", price=100, volume=15, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
        print_order_response(response)
        response: InsertOrderResponse = exchange.insert_order("C2_GREEN_ENERGY_ETF", price=120, volume=10, side=SIDE_BID, order_type=ORDER_TYPE_IOC)
        print_order_response(response)
        '''
        response: InsertOrderResponse = exchange.insert_order("C1_GAS_INC", price=100001, volume=15, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
        print_order_response(response)
        p = exchange.get_last_price_book("C1_GAS_INC")
        print(p)
        print(exchange.get_outstanding_orders("C1_GAS_INC"))
        exchange.delete_orders("C1_GAS_INC")
        print(exchange.get_outstanding_orders("C1_GAS_INC"))
        
        time.sleep(60)
        # exchange.insert_order("C1_GAS_INC", price=100, volume=16, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
