...
def trade_cycle(e: Exchange, vol: int,bought_prc: int,sell_prc: int):
    ...
    if basket_book and basket_book.bids and basket_book.asks:
        ...
        if bought_prc < sell_prc:
            response: InsertOrderResponse = e.insert_order(BASKET_INSTRUMENT_ID, price=new_ask, volume=vol*2, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
        if response.success:
...
def main():
    ...
    while True:
        trade_cycle(e,vol,bought_prc,sell_prc)
