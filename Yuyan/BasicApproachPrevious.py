...
def main():
    ...
    b_pre_b_price = 1000000
    b_pre_s_price = 0
    s1_pre_b_price = 1000000
    s1_pre_s_price = 0
    s2_pre_b_price = 1000000
    s2_pre_s_price = 0
    counter1=0
    counter0=0
    ...
    while True:
        ...
        if result[0]==0:
            ...
            if positions_and_cash[case[0]]["volume"] - volume_basket <= -400:
                exchange.delete_orders(case[0])
            ...
            if positions_and_cash[case[1]]["volume"] + volume_stock >= 400:
                exchange.delete_orders(case[1])
            ...
            if positions_and_cash[case[2]]["volume"] + volume_stock >= 400:
                exchange.delete_orders(case[2])
            ...
            if result[7] > 1 and counter0<=9:
                response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1]-0.2, volume=10, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
                response: InsertOrderResponse = exchange.insert_order(case[1], price=result[3]+0.2, volume=5, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
                response: InsertOrderResponse = exchange.insert_order(case[2], price=result[5]+0.2, volume=5, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
        ...
        if result[0]==1:
            ...
            if result[7] > 1 and counter0<=9:
                counter0+=1
                response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1]-0.2, volume=10, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
                response: InsertOrderResponse = exchange.insert_order(case[1], price=result[3]-0.2, volume=5, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
                response: InsertOrderResponse = exchange.insert_order(case[2], price=result[5]-0.2, volume=5, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
