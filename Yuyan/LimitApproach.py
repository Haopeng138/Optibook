    ...
    while True:
        ...
        if result[0]==0:
            ...
            if (result[7]>2):
                ...
                response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1], volume=40, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
                # print_order_response(response)
                response: InsertOrderResponse = exchange.insert_order(case[1], price=result[3], volume=20, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
                # print_order_response(response)
                response: InsertOrderResponse = exchange.insert_order(case[2], price=result[5], volume=20, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
            
            # Start buying stock and selling basket
            response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1]-0.1, volume=volume_basket, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
            # print_order_response(response)
        ...
        if result[0]==1:
            ...
            if (result[7]>2):
                ...
                response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1], volume=40, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
                # print_order_response(response)
                response: InsertOrderResponse = exchange.insert_order(case[1], price=result[3], volume=20, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
                # print_order_response(response)
                response: InsertOrderResponse = exchange.insert_order(case[2], price=result[5], volume=20, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
