...
def main():
    ...
    while True:
        ...
        offsetBuy = 0.1
        offsetSell = 0.1
        offsetBuyBacket = 0.15
        offsetSellBacket = 0.15
        if result[0]==0:
            ...
            response: InsertOrderResponse = exchange.insert_order(case[0], price=result[1]-result[7]*offsetSellBacket, volume=volume_basket, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
            # print_order_response(response)
            response: InsertOrderResponse = exchange.insert_order(case[1], price=result[3]+result[7]*offsetBuy, volume=volume_stock, side=SIDE_BID, order_type=ORDER_TYPE_IOC)
            # print_order_response(response)
            response: InsertOrderResponse = exchange.insert_order(case[2], price=result[5]+result[7]*offsetBuy, volume=volume_stock, side=SIDE_BID, order_type=ORDER_TYPE_IOC)
